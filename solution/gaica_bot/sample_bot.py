...
from gaica_bot.floors import id_to_level_walls, id_to_level_breakable
...
class SampleBot:
    ...
    def on_tick(self, message: TickMessage) -> BotCommand:
        ...
        pickup = False
        kick = False
        print("LEVEL:", self.state.level.identifier)
        print("BOT ALIVE AND GOIN TO SMTH", me)
        ...
        if not has_weapon and nearest_pickup is not None:
            ...
            print("BOT GOING TO KILL APONENT")
            to_enemy = Vec2(
                enemy.position.x - me.position.x,
                enemy.position.y - me.position.y,
            )
        ...
        # if move:
        #     cetner_cell = self.cell_map.cell_for_position(move)
        #     if cetner_cell:
        #         move = Vec2(*self.cell_map.cell_center(*cetner_cell))
    ...
    def _is_step_walkable(self, origin: Vec2, move: Vec2) -> bool:
        ...
        origin_x, origin_y = self.cell_map.cell_for_position(origin)
        cell_x_to, cell_y_to = self.cell_map.cell_for_position(probe)

        level_map = id_to_level_walls[self.state.level.identifier]
        level_break = id_to_level_breakable[self.state.level.identifier]
        orgin_cell = origin_y * self.cell_map.walkable_mask.shape[1] + (origin_x + 1)
        to_cell = cell_y_to * self.cell_map.walkable_mask.shape[1] + (cell_x_to + 1)
        print("FROM TO", orgin_cell, to_cell)

        wall_between = not ((to_cell, orgin_cell) in level_map or (orgin_cell, to_cell) in level_map)
        break_between = ((to_cell, orgin_cell) in level_break or (orgin_cell, to_cell) in level_break)
        reahable = wall_between | break_between

        print("WALL BETWEEN?:", wall_between)
        print("WALL IS BREAKABLE?:", break_between)
        print("Reachable?:", reahable)

        return reahable and self.cell_map.is_floor_cell(origin_x, origin_y)
